...
public class MainActivity extends Activity {
            ...
            public void run() {
                ...
                try {
                    ...
                    final Bitmap b = Bitmap.createScaledBitmap(
                            BitmapFactory.decodeResource(getResources(), R.drawable.map, options),
                            (int) (iw * scale), (int) (ih * scale), true);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (telemetryConfig.getTelemetryLevel().equals(
                            TelemetryConfig.LEVEL_DISABLED)) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ((TextView) findViewById(R.id.fail_text)).setText(
                                    getString(R.string.initFail_configError) + ": "
                                            + e.getMessage());
    ...
    private void page_serverSelect(TestPoint selected, TestPoint[] servers) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_dropdown_item, options.toArray(new String[0]));
        ...
        st.start(new Speedtest.SpeedtestHandler() {
            ...
            public void onDownloadUpdate(final double dl, final double progress) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ((TextView) findViewById(R.id.dlText)).setText(
                                progress == 0 ? "..." : format(dl));
                        ((GaugeView) findViewById(R.id.dlGauge)).setValue(
                                progress == 0 ? 0 : mbpsToGauge(dl));
                        ((ProgressBar) findViewById(R.id.dlProgress)).setProgress(
                                (int) (100 * progress));
                    }
                });
            }
            ...
            public void onUploadUpdate(final double ul, final double progress) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ((TextView) findViewById(R.id.ulText)).setText(
                                progress == 0 ? "..." : format(ul));
                        ((GaugeView) findViewById(R.id.ulGauge)).setValue(
                                progress == 0 ? 0 : mbpsToGauge(ul));
                        ((ProgressBar) findViewById(R.id.ulProgress)).setProgress(
                                (int) (100 * progress));
                    }
                });
            ...
            @Override
            public void onPingJitterUpdate(final double ping, final double jitter,
                    final double progress) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ((TextView) findViewById(R.id.pingText)).setText(
                                progress == 0 ? "..." : format(ping));
                        ((TextView) findViewById(R.id.jitterText)).setText(
                                progress == 0 ? "..." : format(jitter));
                    }
                });
            }
            ...
            public void onEnd() {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        saveToDB(getIntent().getExtras().getString("name_wifi"),
                                dow.getText().toString(), upl.getText().toString(), new Date());
                        DBManager dbManager = new DBManager(getApplicationContext());
                        dbManager.open();

                        Cursor cursor = fetch();
                        Toast.makeText(com.example.wifianalyzer.ui.MainActivity.this,
                                "The total cursor count is " + cursor.getCount(),
                                Toast.LENGTH_LONG).show();
                        SpeedTestAdapter speedTestAdapter = new SpeedTestAdapter(
                                com.example.wifianalyzer.ui.MainActivity.this, cursor);
                ...
                new Thread() {
                    public void run() {
                        while (System.currentTimeMillis() < endT) {
                            final double f =
                                    (double) (System.currentTimeMillis() - startT) / (double) (endT
                                            - startT);
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
    ...
    private void saveToDB(String name, String d, String u, Date date) {
        ...
        try {
            ...
            database.execSQL(
                    "INSERT INTO " + SampleSQLiteDBHelper.PERSON_TABLE_NAME + " VALUES(" + name
                            + "," + d.replaceAll(",", ".") + "," + u.replaceAll(",", ".") + ","
    ...
    public Cursor fetch() {
        Cursor cursor = this.database.query(SampleSQLiteDBHelper.PERSON_TABLE_NAME,
                new String[]{SampleSQLiteDBHelper.PERSON_COLUMN_DATE,
                        SampleSQLiteDBHelper.PERSON_COLUMN_D,
                        SampleSQLiteDBHelper.PERSON_COLUMN_NAME}, null, null, null, null, null);
        if (cursor != null) {
            cursor.moveToFirst();
        }
        return cursor;
